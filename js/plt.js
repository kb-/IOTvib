...
export function surf(divId,x,y,z,type,zopt){//"id",1d[],1d[],2d[]
  ...
  if(zopt.type=="log"){
    ...
    layout.zaxis.tickvals = logbinslog;
    layout.zaxis.ticktext = logbins;
  ...
  data.colorbar.title = {text:unit_conversion[displayed_spectrogram_idx].u};
...
export function updateSurf(divId,x,y,z,zopt){//"id",1d[],1d[],2d[]
  ...
  update.colorbar.title = {text:unit_conversion[displayed_spectrogram_idx].u};
...
function colorbar(minval, maxval){
  ...
  var colorbar = {
    ...
    ticktext: logbins.concat([minval.toExponential(2), maxval.toExponential(2)]).filter((e,i)=>(i != minbin && i != maxbin)),
  }
