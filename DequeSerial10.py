...
async def startWS1():
    return await websockets.serve(useB, "127.0.0.1", 5678)  

async def startWS2():
    bound_useBfft = functools.partial(useBfft, spec = spectrum)
    return await websockets.serve(bound_useBfft, "127.0.0.1", 5679)
  
try:
    thread = threading.Thread(target=read_from_port, args=(ser,buffer.append,buffer2.append))
    thread.start()
    
    print(asyncio.iscoroutinefunction(startWS1)) #True !!!
    print(asyncio.iscoroutinefunction(startWS2)) #True !!!
   
    loop = asyncio.get_event_loop()
#    loop.create_task(asyncio.coroutine(startWS1)) #didn't help
#    loop.create_task(asyncio.coroutine(startWS2)) #didn't help
    loop.create_task(startWS1)#fail !#@!& TypeError: a coroutine was expected, got <function startWS1 at 0x0000004AE29DA0D0>
#    loop.create_task(startWS2)
    loop.run_forever()
