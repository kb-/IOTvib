...
class spectral:
    def __init__(self, fs, nlines, overlap=0.75, win='hann', averaging='overall',nAverage=np.inf, winParam=None):
        ...
        if winParam == None:
            ...
            bound = functools.partial(getattr(sig,self.win),self.nl,**winParam)
...
#        print(self.m)
    ...
    #spectral rms: sum((Y*ECF)**2*1/2)**(1/2)), with Y amplitude spectrum (positive frequencies half)
    def getECF(self):
