//Generate signal data at 3200 Hz, send unscaled data in Serial binary packets
//
...
//      data.cnt = SysTick->VAL+7500;//accuracy testing (unit=clock cycle, contant = perfect accuracy)
      ...
      data.x = (int16_t)(0.75*pow(2,13)*sin(2*pi*50*t_)+0.25*pow(2,13)*sin(2*pi*20*t_));//The X value is stored in values[0] and values[1].*(1.5+0.1*sin(2*pi*0.005*t_))
      data.y = (int16_t)(0.5*pow(2,13)*sin(2*pi*50*t_)+0.15*pow(2,13)*sin(2*pi*20*t_));;//(int16_t)(0.25*pow(2,13)*sin(2*pi*500*t_*(1.5+0.1*sin(2*pi*0.02*t_))));//The Y value is stored in values[2] and values[3].
      data.z = 0;//(int16_t)(0.5*pow(2,13)*sin(2*pi*300*t_*(1.5+0.1*sin(2*pi*0.05*t_))));//The Z value is stored in values[4] and values[5].
