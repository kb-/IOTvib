...
fftlines = fs/df
fft_settings = {"fs":fs, "nlines":fftlines, "overlap":overlap, "win":fftwindow, "averaging":fftaveraging, "nAverage":nAverage, "winParam":{"sym":True}}
...
#            ser.close()
        ...
        if data["action"] == "settingsFFT":
            FFTsettings = formatFFTsettings(data["FFTsettings"])
            
            import pdb; pdb.set_trace()
#            spectrum = setSpectrum(FFTsettings)

def formatFFTsettings(d):
    FFTsettings_ = {}
    for k in d:
        print(k, k.replace("fft-",""))
        FFTsettings_[k.replace("fft-","")]  = FFTsettings[k]
    return FFTsettings_
